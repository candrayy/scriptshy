  ...
  <body>
      ...
      <div id="content" class="p-4 p-md-5 pt-5">
        ...
        <div class="shadow p-3 mb-1 bg-white rounded">
          <h1 class="mb-4">Data Platform</h1>
          <a href="{{ route('tambah-data-platform') }}" class="btn p-auto rounded-pill mb-3 btn-primary">Tambah Data</a>
          <table class="table table-hover table-bordered">
              <thead>
                  <tr>
                      <th scope="col">No.</th>
                      <th scop="col">Kode Platform</th>
                      <th scop="col">Platform</th>
                      <th scop="col">Aksi</th>
                  </tr>
                  @foreach ($dtPlatform as $item)
              </thead>
              <tbody>
                  <tr>
                      <td>{{ $loop->iteration }}</td>
                      <td>{{ $item->kd_platform }}</td>
                      <td>{{ $item->platform }}</td>
                      <td>
                        <a href="{{ route('edit-data-platform', $item->id) }}"><span class="fa fa-edit" style="color: limegreen"></span></a>
                        |
                        <a href="{{ route('delete-data-platform', $item->id) }}" onclick="return confirm('Yakin Data Mau Dihapus?')"><span class="fa fa-trash" style="color: red"></span></a>
                      </td>
                  </tr>
                  @endforeach
              </tbody>
