  ...
  <body>
      ...
      <div id="content" class="p-4 p-md-5 pt-5">
        ...
        <div class="shadow p-3 mb-1 bg-white rounded">
          <h1 class="mb-4">Data Hashtag</h1>
          <!-- <p>#YT #YTRewind</p> -->
          <a href="{{ route('tambah-data-hashtag') }}" class="btn p-auto rounded-pill mb-3 btn-primary">Tambah Data</a>
          <table class="table table-hover table-bordered">
              <thead>
                  <tr>
                      <th scope="col">No.</th>
                      <th scop="col">Kode Hashtag</th>
                      <th scop="col">Hashtag</th>
                      <th scop="col">Aksi</th>
                  </tr>
                  @foreach ($dtHashtag as $item)
              </thead>
              <tbody>
                  <tr>
                      <td>{{ $loop->iteration }}</td>
                      <td>{{ $item->kd_hashtag }}</td>
                      <td>{{ $item->hashtag }}</td>
                      <td>
                        <a href="{{ route('edit-data-hashtag', $item->id) }}"><span class="fa fa-edit" style="color: limegreen"></span></a>
                        |
                        <a href="{{ route('delete-data-hashtag', $item->id) }}" onclick="return confirm('Yakin Data Mau Dihapus?')"><span class="fa fa-trash" style="color: red"></span></a>
                      </td>
                  </tr>
                  @endforeach
              </tbody>
